...
@router.put("/contacts/{id}", response_model=contacts.ContactRead)
def update_contact(id: str, contact: contacts.ContactCreate, db: Session = Depends(database.get_db),get_current_user: user.newuser=Depends(get_current_user)):
    db_contact = db.query(Contacts.Contact).filter(Contacts.Contact.id == id,Contacts.Contact.user_id==get_current_user.id).first()
